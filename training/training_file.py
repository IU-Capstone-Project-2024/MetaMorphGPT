...
def make_dataset_for_training(json_filename, count=-1):  # old function
    ...
    dataset = [entry for entry in Vfile if
               isinstance(entry["previous_message"], str) and isinstance(entry["next_message"], str)]
    if count > 0:
...
def make_dataset_from_row_data(json_filename, username, count=-1):
    ...
    dataset = [entry for entry in updated_dataset if
               isinstance(entry["previous_message"], str) and isinstance(entry["next_message"], str)]
    if count > 0:
        dataset = dataset[:count]
    dataset = Dataset.from_list(dataset)
...
def train_model_sync(zip_name, model_name, username, num_of_epochs=3, count=-1, drive=drive):
    """
    ...
    Args:
        ...
        model_name (str): The directory where the trained model will be saved.(db_models/{model_name})
...
async def train_model_async(zip_name, model_name, username, num_of_epochs=3, count=-1, drive=drive):
    """
    Асинхронная обертка для функции тренировки модели.
    """
    loop = asyncio.get_event_loop()
    with concurrent.futures.ThreadPoolExecutor() as pool:
        await loop.run_in_executor(pool, train_model_sync, zip_name, model_name, username, num_of_epochs, count, drive)

# train_model("datasets/ontyaga.json", "db_models/ontyaga_4ep", num_of_epochs=4)
